...
public final class Queen extends Piece{
    ...
    public Collection<Move> calculateLegalMoves(final Board board) {
        ...
        for (final int CoordinateOFFSET : MOVE_VECTOR_COORDINATE) {
            ...
            while (BoardUtils.UTILS.isValidTileCoordinate(candidateDestinationCoordinate)) {
    ...
    private static boolean isFirstColumnExclusion(final int currentPosition, final int candidateOFFSET) {
        return BoardUtils.UTILS.FIRST_COLUMN.get(currentPosition) && (candidateOFFSET == -9 || candidateOFFSET == 7 || candidateOFFSET == -1);
    }

    private static boolean isEighthColumnExclusion(final int currentPosition, final int candidateOFFSET) {
        return BoardUtils.UTILS.EIGHTH_COLUMN.get(currentPosition) && (candidateOFFSET == 9 || candidateOFFSET == -7 || candidateOFFSET == 1);
    }
}