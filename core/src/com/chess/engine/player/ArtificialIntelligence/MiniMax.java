...
public final class MiniMax {
                ...
                return Ordering.from(new Comparator<Move>() {
                    ...
                    public int compare(final Move move1, final Move move2) {
                        return ComparisonChain.start()
                                ...
                                .compare(BoardUtils.UTILS.mostValuableVictimLeastValuableAggressor(move2), BoardUtils.UTILS.mostValuableVictimLeastValuableAggressor(move1))
                                .result();
                    }
        ...
        EXPENSIVE_SORT {
            ...
            Collection<Move> sort(final Collection<Move> moves) {
                return Ordering.from(new Comparator<Move>() {
                    ...
                    public int compare(final Move move1, final Move move2) {
                        return ComparisonChain.start()
                                .compareTrueFirst(BoardUtils.UTILS.kingThreat(move1), BoardUtils.UTILS.kingThreat(move2))
                                .compareTrueFirst(move1.isCastlingMove(), move2.isCastlingMove())
                                .compare(BoardUtils.UTILS.mostValuableVictimLeastValuableAggressor(move2), BoardUtils.UTILS.mostValuableVictimLeastValuableAggressor(move1))
                                .result();
                    }
            ...
            if (moveTransition.getMoveStatus().isDone()) {
                executorService.execute(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (MiniMax.this.gameEndDueToTimeOut) {
    ...
    private int max(final Board board, final int depth, final int highest, final int lowest) {
        ...
        if (depth == 0 || BoardUtils.UTILS.isEndGameScenario(board)) {
    ...
    private int calculateQuiescenceDepth(final Board toBoard, final int depth) {
        if(depth == 1 && this.quiescenceCount < MAX_QUIESCENCE) {
            ...
            for(final Move move: BoardUtils.UTILS.lastNMoves(toBoard, 2)) {
                if(move.isAttack()) {
                    activityMeasure += 1;
                }
            }
